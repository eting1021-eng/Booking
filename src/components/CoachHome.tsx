...
export default function CoachHome({ sessions, bookings, profile, allUsers = {} }: CoachHomeProps) {
  ...
  // Sync editProfile when the loaded profile prop changes
  React.useEffect(() => {
    if (profile && profile.name !== '載入中...') {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        setEditProfile(prev => ({...prev, imageUrl: compressedBase64}));
        setSaveMessage('頭像上傳處理完成！請記得點擊最下方「儲存首頁設定」按鈕進行儲存。');
        alert('頭像上傳處理完成！請記得點擊最下方「儲存首頁設定」按鈕進行儲存。');
      } catch (err) {
        setSaveMessage('頭像處理失敗，請重試');
        alert('頭像處理失敗，請重試');
      } finally {
  ...
  const handleExperiencePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (filesArray.length > remainingSlots) {
        setSaveMessage(`經歷照片上限為 4 張，已自動為您處理前 ${remainingSlots} 張`);
      }
      ...
      try {
        ...
        if (validImages.length > 0) {
          ...
          const successMsg = `成功處理 ${validImages.length} 張照片！請記得點擊最下方「儲存首頁設定」按鈕進行儲存。`;
    ...
    return (
      <div className="flex flex-col h-full px-8 pt-10 pb-4 animate-in fade-in">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => { setShowAddForm(false); setEditingSessionId(null); setIsCopying(false); }} className="text-[#7A7A7A] p-2 hover:bg-gray-100 rounded-full">✕</button>
        </div>
        {isCopying && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs text-amber-800 font-bold mb-3 flex items-center gap-2 animate-in slide-in-from-top-2 duration-300">
          ...
          <>
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-2">
                ...
                <div className="flex bg-[#F6F5F2] p-1 rounded-xl border border-[#C9D6D0]/50 shrink-0">
                  <button 
                    id="coach-view-list-toggle"
                    type="button"
                    onClick={() => setViewMode('list')}
                    className={`px-2.5 py-1 text-[10px] font-bold rounded-lg transition-colors ${viewMode === 'list' ? 'bg-[#7FA8A4] text-white shadow-sm' : 'text-[#7A7A7A] hover:text-[#2F3437]'}`}
                  >
                    列表
                  </button>
                  <button 
                    id="coach-view-calendar-toggle"
                    type="button"
                    onClick={() => setViewMode('calendar')}
                    className={`px-2.5 py-1 text-[10px] font-bold rounded-lg transition-colors ${viewMode === 'calendar' ? 'bg-[#7FA8A4] text-white shadow-sm' : 'text-[#7A7A7A] hover:text-[#2F3437]'}`}
                  >
                    月曆
                  </button>
                </div>
              ...
              <button 
                type="button"
                onClick={() => viewMode === 'calendar' ? handleOpenAddOnDate(selectedDate) : handleOpenAdd()} 
                className="text-[#D9A46A] font-bold text-xs bg-[#D9A46A]/10 px-3 py-1.5 rounded-full hover:bg-[#D9A46A]/20 transition-colors shrink-0"
              >
                {viewMode === 'calendar' ? `+ 新增此日時段` : `+ 新增時段`}
              </button>
            </div>
            ...
            {(() => {
              ...
              if (filteredSessions.length === 0) {
                return (
                  <div className="text-center py-10 bg-[#F6F5F2]/50 rounded-3xl border border-dashed border-[#C9D6D0] p-6 text-sm text-[#7A7A7A] font-bold flex flex-col items-center justify-center">
                    <span>
                      {viewMode === 'calendar' 
                        ? "本日尚無課程時段。" 
                        : "目前尚無近期課程，點擊上方新增時段。"
                    ...
                    {viewMode === 'calendar' && (
                      <button 
                        ...
                        className="mt-3 bg-[#7FA8A4] text-white px-4 py-2 rounded-xl font-bold text-xs shadow-md hover:bg-[#6f9793] active:scale-95 transition-all"
                      >
                        + 在此日期新增時段
                      </button>
                    )}
              ...
              return filteredSessions.map((session) => {
                ...
                return (
                  <div key={session.id} className={`bg-white rounded-3xl p-5 border border-[#C9D6D0] shadow-sm mb-4 relative overflow-hidden ${isCompleted ? 'opacity-80' : ''}`}>
                    <div className="flex justify-between items-start mb-2">
                      <h3 className={`font-bold text-[#2F3437] ${isCompleted ? 'line-through decoration-[#7A7A7A]' : ''}`}>{session.title}</h3>
                      <div className="flex gap-2 items-center">
                        <span className="text-[#7FA8A4] font-bold text-sm">${session.price}</span>
                        {isCompleted ? (
                          <span className="bg-[#E0EBE8] text-[#7FA8A4] px-2.5 py-1 rounded text-xs font-bold">已完成</span>
                        ) : (
                          <>
                            <button onClick={() => handleDuplicateSession(session)} className="font-bold text-xs p-1.5 px-3 rounded transition-colors text-amber-500 hover:text-amber-700 bg-amber-50 hover:bg-amber-100 cursor-pointer" title="複製此時段">
                              複製
                            </button>
                            <button onClick={() => handleEditSession(session)} className="font-bold text-xs p-1.5 px-3 rounded transition-colors text-blue-400 hover:text-blue-600 bg-blue-50 hover:bg-blue-100 cursor-pointer" title="編輯課程">
                              編輯
                            </button>
                            <button onClick={() => handleDeleteSession(session.id)} className={`font-bold text-xs p-1.5 px-3 rounded transition-colors cursor-pointer ${isDeleting ? 'bg-red-500 text-white shadow-sm' : 'text-red-400 hover:text-red-600 bg-red-50 hover:bg-red-100'}`} title="刪除課程">
                              {isDeleting ? '確認刪除' : '✕'}
                            </button>
                          </>
                        )}
        ...
        {activeTab === 'profile' && (
          <form onSubmit={handleSaveProfile} className="space-y-4 animate-in fade-in">
             <div className="bg-white p-5 rounded-3xl border border-[#C9D6D0] shadow-sm space-y-4">
              ...
              <div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                    ...
                    {isUploadingPhotos ? (
                      <div className="aspect-square rounded-xl border-2 border-dashed border-[#C9D6D0] flex flex-col items-center justify-center text-[#7A7A7A] bg-[#F6F5F2] animate-pulse">
                        <span className="text-xs font-bold text-[#7FA8A4]">處理中...</span>
                      </div>
              ...
              <div>
                <label className="block text-sm font-bold text-[#7A7A7A] mb-1">聯絡方式 (LINE, 電話, Email等)</label>
                <textarea 
                  value={editProfile.contactInfo || ''} 
                  onChange={e => setEditProfile({...editProfile, contactInfo: e.target.value})} 
                  className="w-full bg-[#F6F5F2] border border-[#C9D6D0] rounded-xl px-4 py-3 text-[#2F3437] focus:outline-none focus:border-[#7FA8A4] mb-3" 
                  rows={3}
                  placeholder="請輸入您的聯絡方式，讓學生可以找到您..."
                />
              </div>
              <div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="relative w-full">
                      <input 
                        ...
                        className="w-full bg-[#F6F5F2] border border-[#C9D6D0] rounded-xl px-4 py-3 text-[#2F3437] focus:outline-none focus:border-[#7FA8A4] text-sm disabled:opacity-50" 
                      />
                      {isUploadingAvatar && (
                        <div className="absolute inset-0 bg-[#F6F5F2]/80 flex items-center justify-center rounded-xl">
                          <span className="text-xs font-bold text-[#7FA8A4] animate-pulse">頭像處理中...</span>
                        </div>
                      )}
                    </div>
              ...
              <button 
                type="submit" 
                disabled={isSavingProfile || isUploadingAvatar || isUploadingPhotos}
                className="w-full bg-[#7FA8A4] text-white py-4 rounded-2xl font-bold shadow-md mt-4 active:scale-[0.98] transition-transform disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isSavingProfile ? (
                  <>
                    <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
                    <span>正在儲存設定中，請稍候...</span>
                  </>
                ) : (
                  '儲存首頁設定'
                )}
