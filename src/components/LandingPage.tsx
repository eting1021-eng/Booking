      ...
      {profile.contactInfo && (
        <div className="absolute top-6 right-6 z-30 flex flex-col items-end">
          <button 
            ...
            className="px-4 py-2.5 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center gap-2 text-white hover:bg-white/30 transition-colors shadow-sm border border-white/30 font-bold text-sm"
          >
            {showContact ? (
              <>
                <X size={18} />
          ...
          {showContact && (
            <div className="mt-3 w-56 bg-white rounded-2xl p-4 shadow-xl border border-[#C9D6D0] animate-in fade-in slide-in-from-top-2 origin-top-right text-left">
              <h3 className="text-[11px] font-bold text-[#D9A46A] mb-2 tracking-widest border-b border-[#F6F5F2] pb-2">聯絡教練</h3>
              <p className="text-[#2F3437] text-[13px] leading-relaxed whitespace-pre-wrap font-medium">
                {profile.contactInfo}
              </p>
            </div>
          )}
